  ...
  <div>
    <el-dialog top='4vh' :close-on-click-modal=false title="修改实施阶段" :visible="visible" :before-close="handleCancel" @update:visible="$emit('update:visible', $event)">
      <el-form label-width="120px" class="mb-22" :model="form">
        ...
        <el-form-item>
          <el-tag v-for="item in selectedDeviceList" :key="item.roleName" :type="item.type" style="margin:0px 10px">
            {{item.roleName}}
          </el-tag>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
          ...
          if (this.selectedDeviceList.length > 0) {
            for (var i = 0; i < this.selectedDeviceList.length; i++) {
              for (var j = 0; j < this.projects.length; j++) {
                if (this.selectedDeviceList[i] == this.projects[j].id) {
                  list1.push(this.projects[j])
                }
              }
            }
          }
          this.selectedDeviceList = list1
        }
      })
    },
    toggleSelection() {
      // if (this.selectedDeviceList.createTime) {
      //   const list1 = []
      //   if (this.selectedDeviceList.length > 0) {
      //     for (var i = 0; i < this.selectedDeviceList.length; i++) {
      //       for (var j = 0; j < this.projects.length; j++) {
    ...
    init(val) {
      ...
      this.getRoleList(val.enableUserList) //客户列表
      if (val.linkType == 1) {
        this.selectModelDict() // 查客户下模型

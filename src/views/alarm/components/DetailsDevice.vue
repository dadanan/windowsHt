      ...
      <el-form label-width="120px" class="mb-22" :model="form">
        ...
        <el-form-item label="选择关联">
          <template>
            <el-radio-group v-model="form.linkType">
              <el-radio :label="0" disabled>不关联</el-radio>
              <el-radio :label="1" disabled>关联设备</el-radio>
              <el-radio :label="2" disabled>关联工程</el-radio>
            </el-radio-group>
          </template>
        </el-form-item>
        <el-form-item>
          ...
          <template v-if="form.linkType == 1">
            <el-select v-model="form.linkDeviceModelId" style="width:40%" @change="selectByModelId" disabled>
              <el-option v-for='item in devList' :label="item.name" :value="item.id" :key='item.id' ></el-option>
            </el-select>
            <el-select v-model="form.linkDeviceId" style="width:40%" disabled>
              <el-option v-for='item in devilist' :label="item.name" :value="item.id" :key='item.id' ></el-option>
            </el-select>
          </template>
          <template v-if="form.linkType == 2">
            <el-select v-model="form.linkProjectId" style="width:100%" disabled>
        ...
        <el-form-item>
          ...
          <template v-if="form.cycleType == 1">
            <span>每</span>
            <el-input v-model="form.cycleNums" style="width:10%" disabled></el-input>
            <span>月告警日期</span>
            <el-date-picker v-model="form.nextExecuteTime" type="date" value-format="timestamp" placeholder="选择日期" :picker-options="pickerOptions0" disabled style="width:40%; margin-right:20px">
            </el-date-picker>

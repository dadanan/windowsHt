...
import { createDeviceModel } from '@/api/device/config'
...
export default {
  ...
  methods: {
    ...
    createDeviceModel() {
      // 调整第三步「硬件功能项」的数据结构
      const newArray = this.abilityList.map(item => {
        return {
          ablityId: item.ablityId,
          definedName: item.definedName,
          maxVal: item.maxVal,
          minVal: item.minVal,
          deviceModelAblityOptions: item.deviceAblityOptions.map(iItem => {
            return {
              ablityOptionId: iItem.id,
      ...
      // 调整第四步「版式配置」的数据结构
      const modelFormatPages = this.pageOfForamt.map(item => {
        return {
          pageId: item.pageNo,
          showName: item.name,
          showStatus: item.showStatus ? 1 : 0,
          modelFormatItems: item.wxFormatItemVos.map(iItem => {
            return {

<template>
  <div>
    <el-dialog top='4vh' :close-on-click-modal=false title="添加计划" :visible="visible" :before-close="handleCancel" @update:visible="$emit('update:visible', $event)">
  ...
  * 系统发布任务即视为计划预警，任务发布日至任务截止日之间为任务实际筹备及执行期限；</span></p>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click='addPlanModel'>确定</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    addPlanModel() {
      ...
      addPlanModel(this.form).then(res => {
        if (res.code === 200) {
          this.$message({
            type: 'success',
            message: '添加成功!'
